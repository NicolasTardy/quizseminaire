...
console.log("Firebase config utilisée :", firebaseConfig);
...
const QUIZ_QUESTIONS = [
  {
    ...
    correct: "Lui offrir une boîte vide",
    image: "/image/imageintro.png"
  },
  {
    question: "En 2024, quelle part de la marge service est représentée par les services par abonnement ?",
    options: ["50%", "65%", "82%", "30%"],
    correct: "82%",
    image: "/image/Q2.png"
  },
  {
    question: "Quel service a connu une forte croissance entre 2021 et 2023 ?",
    options: ["Hubside", "Darty Max", "Homeserve", "Nespresso"],
    correct: "Darty Max",
    image: "/image/Q4.png"
  },
  {
    question: "Quel est le canal de vente dominant pour les services en 2024 ?",
    options: ["Web", "Magasins franchisés", "Magasins intégrés", "Télévente"],
    correct: "Magasins intégrés",
    image: "/image/Q7.png"
  },
  {
    question: "Quelle est l’ambition de croissance de la marge services entre 2024 et 2025 ?",
    options: ["+20M€", "+60M€", "+100M€", "+10M€"],
    correct: "+60M€",
    image: "/image/Q3.png"
  },
  {
    question: "Quel est l’un des leviers identifiés pour améliorer la fidélisation des abonnés ?",
    options: [
      "Réduction du nombre de services",
      "Suppression des offres web",
      "Mise en place d’un plan anti-churn",
      "Augmentation des prix"
    ],
    correct: "Mise en place d’un plan anti-churn",
    image: "/image/souri.png"
  },
  {
    question: "Quelle catégorie de produit présente un fort potentiel de croissance pour les services ?",
    options: ["Téléphonie", "Électroménager", "Informatique", "Jeux vidéo"],
    correct: "Téléphonie",
    image: "/image/Q6.png"
  },
  {
    question: "Quel canal est en développement et jugé prometteur pour la vente de services ?",
    options: [
      "RCD (Relation Client à Distance)",
      "Bornes interactives",
      "Télévision connectée",
      "Réseaux sociaux"
    ],
    correct: "RCD (Relation Client à Distance)",
    image: "/image/Q5.png"
  },
  {
    question: "Quel levier est utilisé pour améliorer la rétention des abonnés ?",
    options: [
      "Campagnes CRM personnalisées et selfcare",
      "Réduction des effectifs",
      "Suppression des offres premium",
      "Diminution du nombre de services"
    ],
    correct: "Campagnes CRM personnalisées et selfcare",
    image: "/image/chuck.png"
  },
  {
    question: "Quel segment est identifié comme opportun dans le cadre de la transition énergétique ?",
    options: ["Jeux vidéo", "Laveries collectives", "Traitement de l’air", "Téléphonie mobile"],
    correct: "Traitement de l’air",
    image: "/image/pand.png"
  }
];

const TIME_PER_QUESTION = 20; // secondes
const TIME_FOR_ANSWER = 3;    // secondes
...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      doc(db, "quiz", "gameState"),
      (docSnapshot) => {
        if (docSnapshot.exists()) {
          const data = docSnapshot.data();
          const old = gameStateRef.current;
          const isNewQuestion =
            typeof old.questionIndex === 'number' &&
            data.questionIndex > old.questionIndex;
          ...
          if (isNewQuestion || (data.questionIndex === 0 && old.questionIndex === -1)) {
      ...
      (err) => console.error("onSnapshot(gameState) error:", err)
    );
    return () => unsubscribe();
  }, [isLoggedIn, isAdmin]);
  ...
  // Sync participants
  useEffect(() => {
    const qCol = query(collection(db, "participants"));
    const unsubscribe = onSnapshot(qCol, (querySnapshot) => {
      const users = [];
  ...
  // --- MOTEUR DE JEU (admin uniquement) ---
  useEffect(() => {
    if (!isAdmin) return;
    let timeoutId;

    if (gameState.view === 'quiz' && typeof gameState.questionStartAt === 'number') {
      const remainingMs = (TIME_PER_QUESTION * 1000) - (Date.now() - gameState.questionStartAt);
      const delay = Math.max(0, remainingMs);
      console.log("[ENGINE] quiz→answer dans", delay, "ms");
      timeoutId = setTimeout(async () => {
        try {
          await updateDoc(doc(db, "quiz", "gameState"), { view: 'answer' });
        } catch (e) {
          console.error("updateDoc(view=answer) error:", e);
        }
      }, delay);
    } else if (gameState.view === 'answer') {
      console.log("[ENGINE] answer→next dans", TIME_FOR_ANSWER * 1000, "ms");
      timeoutId = setTimeout(async () => {
        const nextIndex = (gameStateRef.current.questionIndex ?? -1) + 1;
        try {
          if (nextIndex < QUIZ_QUESTIONS.length) {
            await updateDoc(doc(db, "quiz", "gameState"), {
              questionIndex: nextIndex,
              view: 'quiz',
              questionStartAt: Date.now()
            });
          } else {
            await updateDoc(doc(db, "quiz", "gameState"), { view: 'finished' });
          }
        } catch (e) {
          console.error("advance to next error:", e);
        }
      }, TIME_FOR_ANSWER * 1000);
    }

    return () => clearTimeout(timeoutId);
  }, [isAdmin, gameState.view, gameState.questionStartAt]);

  // Timer affiché pour tous
  useEffect(() => {
    if (gameState.view === 'quiz' && typeof gameState.questionStartAt === 'number') {
      const intervalId = setInterval(() => {
        const elapsed = Math.floor((Date.now() - gameState.questionStartAt) / 1000);
        setTimer(Math.max(0, TIME_PER_QUESTION - elapsed));
      }, 500);
      return () => clearInterval(intervalId);
    }
  }, [gameState.view, gameState.questionStartAt]);
  ...
  const handleLogin = async (pseudoInput) => {
    ...
    const qCol = query(participantsRef, where("pseudo", "==", pseudo));
  ...
  const handleStartQuiz = async () => {
    await setDoc(doc(db, "quiz", "gameState"), {
      view: 'quiz',
      questionIndex: 0,
      showResults: false,
      questionStartAt: Date.now()
  ...
  const handleShowResults = async () =>
    updateDoc(doc(db, "quiz", "gameState"), { showResults: true });

  const handleResetQuiz = async () => {
    const participantsSnapshot = await getDocs(collection(db, "participants"));
    participantsSnapshot.forEach(p => {
      updateDoc(doc(db, "participants", p.id), { score: 0, totalTime: 0 });
    });
    await setDoc(doc(db, "quiz", "gameState"), {
      view: 'login',
      questionIndex: -1,
    ...
    switch (gameState.view) {
      case 'quiz':
        if (gameState.questionIndex >= 0 && gameState.questionIndex < QUIZ_QUESTIONS.length) {
          return (
            <QuizPanel
              question={QUIZ_QUESTIONS[gameState.questionIndex]}
      ...
      case 'finished':
        return (
          <div className="text-center animate-fade-in-up">
            <h2 className="text-3xl font-bold mb-4 text-brand-teal">Quiz terminé !</h2>
            <p className="text-lg text-gray-300">En attente de l'affichage des résultats...</p>
          </div>
        );
      default:
    ...
    <div className="w-full max-w-4xl mx-auto bg-brand-purple/60 backdrop-blur-lg rounded-2xl shadow-2xl p-6 md:p-8 border border-brand-light-purple/50 animate-fade-in-up">
      ...
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-bold text-gray-300">
          Question <span className="text-brand-yellow">{questionNumber}</span>/{totalQuestions}
        </h3>
        <div className={`text-3xl font-extrabold rounded-full w-16 h-16 flex items-center justify-center transition-all duration-300 ${timer <= 5 ? 'bg-brand-pink animate-pulse' : 'bg-brand-yellow text-brand-dark'}`}>
          {timer}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {question.options.map(option => (
          <button
            ...
            className={`w-full p-4 rounded-lg text-lg font-semibold transition-all duration-300 border-2 ${getButtonClass(option)}`}
          >
            {option}
          </button>
        ))}
    ...
    <div className="flex items-center justify-center gap-3 mb-6 text-xl text-gray-300">
      ...
      <span>
        <strong className="text-brand-yellow">
          {participants.filter(p => p.pseudo.toLowerCase() !== 'admin').length}
        </strong> participant(s) connecté(s)
      </span>
    </div>
    <div className="flex flex-wrap justify-center gap-3 max-h-56 overflow-y-auto p-4 bg-brand-dark/50 rounded-lg">
      {participants.map(p => p.pseudo.toLowerCase() !== 'admin' && (
        <span key={p.id} className="bg-brand-light-purple px-4 py-2 rounded-full text-base font-medium text-white">
          {p.pseudo}
        </span>
      ))}
    </div>
...
const ResultsPanel = ({ participants }) => {
  ...
  const fastest = [...participants]
    .filter(p => p.pseudo.toLowerCase() !== 'admin' && p.score > 0)
        ...
        {lowest && sorted.length > 1 && (
          <div className="bg-red-900/50 p-4 rounded-lg">
            ...
            <p className="text-gray-400">
              On ne donnera pas de nom, mais il est de <strong className="text-2xl font-bold text-red-300">{lowest.score}</strong> points.
            </p>
          </div>
        )}
      </div>
  ...
  <div className="w-full max-w-3xl mx-auto bg-brand-purple/60 backdrop-blur-lg rounded-2xl shadow-2xl p-8 border border-brand-light-purple/50 text-center animate-fade-in-up">
    ...
    <div className="flex flex-col md:flex-row justify-center items-center gap-4 mb-8">
      ...
      {gameState.view === 'finished' && !gameState.showResults && (
        <button onClick={onShowResults} className="bg-brand-yellow hover:opacity-80 text-brand-dark font-bold py-4 px-8 text-xl rounded-lg shadow-lg transition-transform transform hover:scale-105">
          Afficher les Résultats
        </button>
      )}
      <button onClick={onReset} className="bg-brand-pink hover:opacity-80 text-white font-bold py-2 px-4 rounded-lg shadow-lg transition-transform transform hover:scale-105">
        Réinitialiser
      </button>
    </div>
